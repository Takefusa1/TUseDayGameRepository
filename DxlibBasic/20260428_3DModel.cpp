...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int nCmdShow)
{
	...
	//モデルのポジション
	VECTOR pos = VGet(600.f, 300.0f, -400.f);
	//カメラのポジション
	...
	//モデル読み込み
	model1 = MV1LoadModel("../3DCharacter/PC.mv1");
	//モデルの読み込み失敗
	if (model1 == -1) return -1;
	//立ちのアニメーションモデル読み込み
	anim_nutral = MV1LoadModel("../3DCharacter/Anim_Neutral.mv1");
	if (anim_nutral == -1) { return -1; }
	//走るアニメーションモデル読み込み
	anim_run = MV1LoadModel("../3DCharacter/Anim_Run.mv1");
	...
	while (ProcessMessage() == 0 && CheckHitKey(KEY_INPUT_ESCAPE) == 0)
	{
		...
		MV1SetAttachAnimTime(model1, attachidx, playertime);
